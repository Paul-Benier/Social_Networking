...
if (isset($_SESSION["relationShips"]) && isset($_SESSION["LOGGED_USER_id"])){
    foreach ($users as $user) {
        if (in_array($user['user_id'], $_SESSION["relationShips"]) || $user['user_id'] == $_SESSION["LOGGED_USER_id"]){
            echo '<label for="id_user">' . $user['user_id'] . ' ' . htmlspecialchars($user['first_name']) . ' ' . htmlspecialchars($user['last_name']) . '</label><br><br>';
        }
        else if (in_array($user['user_id'], $_SESSION["relationShipsHold"])){
            echo '<label for="id_user">' . $user['user_id'] . ' ' . htmlspecialchars($user['first_name']) . ' ' . htmlspecialchars($user['last_name']) . ': friend request on hold</label><br><br>';
        }
