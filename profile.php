...
<?php
    if (isset($_SESSION["relationShipsTBC"]) && isset($_SESSION["LOGGED_USER"][0])){
        if (!empty($_SESSION["relationShipsTBC"])){
            echo '<h2>Validate relationships:</h2>';
            foreach ($users as $user) {
                if (in_array($user['user_id'], $_SESSION["relationShipsTBC"]) && $user['user_id'] != $_SESSION["LOGGED_USER"][0]){
    ...
    if (isset($_SESSION["relationShipsHold"]) && isset($_SESSION["LOGGED_USER"][0])){
        if (!empty($_SESSION["relationShipsHold"])){
            echo "<h2>friend request on hold:</h2>";
            foreach ($users as $user) {
                if (in_array($user['user_id'], $_SESSION["relationShipsHold"]) && $user['user_id'] != $_SESSION["LOGGED_USER"][0]){
